...
public class Choose_Tank1 implements Screen, Serializable {
    protected Vector3 mouse;

    private final Texture background;
    private final TankStars game;
    public Choose_Tank1(TankStars game){
        this.game=game;
        background=new Texture("Choose1_1.png");
        mouse=new Vector3();
    }
    @Override
    public void show() {
            ...
            if (Gdx.input.isButtonPressed(Input.Buttons.LEFT)) {
                ...
                if (mouse.y > 320 && mouse.y < 385) {
                    if (mouse.x < 360 && mouse.x > 300) {
                        ...
                        game.setScreen(new Choose_Tank3(this.game));
                        dispose();
                    }
                }
                else if(mouse.x> 480 && mouse.x<720 && mouse.y> 575 && mouse.y< 625){
                    game.setScreen(new Choose2_Tank1(this.game,1));
                    dispose();
                }
            }
        }
